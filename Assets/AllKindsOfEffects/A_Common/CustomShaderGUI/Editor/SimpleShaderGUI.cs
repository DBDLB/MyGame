...
namespace Scarecrow
{
    public class SimpleShaderGUI : ShaderGUI
    {
        ...
        private void ChangeRenderingMode()
        {
            ...
            string[] blendNames = Enum.GetNames(typeof(RenderingMode));

            EditorGUI.BeginChangeCheck();
            mode = (RenderingMode)EditorGUI.Popup(EditorGUILayout.GetControlRect(), "Rendering Mode", (int)mode, blendNames);
            //绘制两个切换Button
            ...
            if (EditorGUI.EndChangeCheck())
            {
                switch (mode)
                {
                    case RenderingMode.Opaque:
                        _SrcBlend.floatValue = (int)UnityEngine.Rendering.BlendMode.One;
                        _DstBlend.floatValue = (int)UnityEngine.Rendering.BlendMode.Zero;
                    ...
                    case RenderingMode.Alpha:
                        _SrcBlend.floatValue = (int)UnityEngine.Rendering.BlendMode.SrcAlpha;
                        _DstBlend.floatValue = (int)UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha;
                        foreach (Material m in _SrcBlend.targets)
                        {
                            m.SetInt("_ZWrite", 0);
                    ...
                    case RenderingMode.OneAplhaDecal:
                        _SrcBlend.floatValue = (int)UnityEngine.Rendering.BlendMode.SrcAlpha;
                        _DstBlend.floatValue = (int)UnityEngine.Rendering.BlendMode.One;
                        foreach (Material m in _SrcBlend.targets)
                        {
                            m.SetInt("_ZWrite", 0);
                            m.SetOverrideTag("RenderType", "Transparent");
